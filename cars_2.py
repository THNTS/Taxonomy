...
if __name__ == '__main__':
    ...
    results = pool.map(calculate, strings_collection[34000:])

    # Close the pool and wait for the work to finish
    pool.close()
    pool.join()

    df = pd.DataFrame(results, index=np.arange(len(results)), columns=leafs)
    #df.to_excel("/Users/daniilbabin/DS/Taxonomy/true.xlsx", sheet_name='first_9999')
    df.to_csv("/Users/daniilbabin/DS/Taxonomy/true_35.csv")
